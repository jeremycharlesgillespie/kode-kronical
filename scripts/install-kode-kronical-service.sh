...
# Always performs a FRESH install by overwriting existing files
...
create_directories() {
    ...
    chmod 755 "$DATA_DIR"
    info "Created fresh user data directory: $DATA_DIR"
}

# Create default config (always overwrite for fresh install)
create_config() {
    CONFIG_FILE="/etc/kode-kronical/daemon.yaml"
    ...
    cat > "$CONFIG_FILE" << EOF
# kode-kronical-daemon configuration
# Generated by install script on $(date)
daemon:
  # All paths use user directory to avoid permission issues
  pid_file: /home/$SUDO_USER/.local/share/kode-kronical/daemon.pid
  log_file: /home/$SUDO_USER/.local/share/kode-kronical/daemon.log
  data_dir: /home/$SUDO_USER/.local/share/kode-kronical
...
EOF
    ...
    info "Created fresh config file: $CONFIG_FILE"
}
...
# Create systemd service file (always overwrite for fresh install)
create_service_file() {
...
# PID file managed by daemon in user directory
Restart=on-failure
RestartSec=10
...
Environment="USER=$SUDO_USER"
Environment="PATH=/home/$SUDO_USER/.venv/bin:/usr/local/bin:/usr/bin:/bin"
WorkingDirectory=/home/$SUDO_USER

# Security settings (relaxed for compatibility)
# NoNewPrivileges=true
# PrivateTmp=true
# ReadWritePaths=/home/$SUDO_USER/.local /home/$SUDO_USER/.config
...
print_usage() {
    ...
    echo "  • Service: /etc/systemd/system/kode-kronical-daemon.service"
    echo "  • Config:  /etc/kode-kronical/daemon.yaml"
    echo "  • Data:    /home/$SUDO_USER/.local/share/kode-kronical/"
    echo
    ...
    echo "  • Store all data in user directory (no permission issues)"
    echo
    ...
    echo
    info "Data location: /home/$SUDO_USER/.local/share/kode-kronical/"
    echo
}
...
main() {
    ...
    create_config      # Create default config if needed
    stop_existing_daemon
    create_service_file
    install_service
    verify_service
